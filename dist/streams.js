...
var Streams = /*#__PURE__*/function () {
  ...
  _createClass(Streams, [{
    ...
    value: function initialize(domNode, kioapi, preferred_width) {
      ...
      window.problem.hidden = true; // let power = prompt("Введите входную мощность");

      var outer = prompt("Введите выходные выходные мощности через точку с запятой. Пример: 1/4;3/4");
      window.problem.hidden = false; // console.log(power);

      console.log(outer.split(";"));
      _js_SETTINGS__WEBPACK_IMPORTED_MODULE_14__["default"].changeProperty("TASK", outer.split(";")); //SETTINGS.changeProperty("START_POWER", power);
